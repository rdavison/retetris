...
type view = {
  ...
  cellHeight: int,
  offset: int,
  background: [`Red|`Orange|`Yellow|`Green|`Blue|`Purple|`White|`Gray|`DarkGray|`Black]
};
...
let updateClock = (state, env) : Game.state => {
  ...
  let speed = Game.speed(state);
  if(clock > speed) {
    {...state, clock: clock -. speed, step: true, flipFlop: ! Game.flipFlop(state)}
  } else {
  ...
  switch(Game.activePiece(state)) {
    ...
    | Some(activePiece) => {
      ...
      if(Board.hasCollision(newPiece, board)) {
        Result.map(Board.freeze(activePiece, board),
          () => {
            let newPiece = Game.makeRandomPiece(board);
            state
